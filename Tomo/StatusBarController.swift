...
class StatusItemView {
    ...
    var textColor = NSColor(red: 1, green: 1, blue: 1, alpha: 1) {
        didSet { tintedIcon = icon?.tinted(with: textColor) }
    }
    ...
    func updateButtonImage(remainingTime: String, completed: CGFloat) {
        ...
        let image = NSImage(size: NSSize(width: width, height: height), flipped: false) { drawRect in
            ...
            Self.drawBox(bgRect: bgRect, completed: completed, bgColor: capturedBgColor)
            if remainingTime == "" {
    ...
    static func drawIcon(bgRect: NSRect, tintedIcon: NSImage?) {
        ...
        let iconRect = NSMakeRect(
            (bgRect.size.width - width) * 0.5 + bgRect.origin.x,
            (bgRect.size.height - height) * 0.5 + bgRect.origin.y,
            width, height
        ).integral
